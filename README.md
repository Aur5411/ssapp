...
一个基于 **Android WebView** 的 Discuz! X3.4 小说论坛壳应用，当前版本 **v1.9.84**。

## 功能特性

- **可配置网址**：首次打开进入设置页填写论坛地址，之后启动直达论坛
- **电脑版网页**：默认桌面 UA，进入网站即为 PC 版完整页面（设置页可关闭）
...
- **App 内下载管理**：文件列表、多选删除、长按重命名、左滑删除、搜索过滤
- **自定义下载目录**：设置页配置，保存于系统 `Download/自定义目录`
- **去广告预设**：菜单/设置页均可开关，匹配 Discuz 常见广告位
- **自定义脚本**：设置页可粘贴任意 JS 脚本（油猴脚本 `@grant none` 版本可直接使用）
